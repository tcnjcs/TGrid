...
/**
 * The TestFileJob Class represents an extremely basic Job that sends out dummy 
 * tasks with files.
 * 
 * @author Stephen
 */
public class TestFileJob implements Job {
    /**
     * Stores a list of all internal Tasks that still need to be sent to clients.
     */
    private LinkedBlockingQueue<TestFileTask> newTasks = new LinkedBlockingQueue<TestFileTask>();
    
    /**
     * Stores a list of all internal Tasks that have failed.
     */
    private LinkedBlockingQueue<TestFileTask> problematicTasks = new LinkedBlockingQueue<TestFileTask>();
    
    /**
     * Stores a list of all internal Tasks have have completed successfully.
     */
    private LinkedBlockingQueue<TestFileTask> finishedTasks = new LinkedBlockingQueue<TestFileTask>();
    ...
    public void storeTask(Task taskToStore)
        ...
        switch(taskToStore.getState())
            ...
            case COMPLETED: //if the task is completed, add it to the completed tasks list
                finishedTasks.add((TestFileTask)taskToStore);
            ...
            case PROBLEMATIC:
                problematicTasks.add((TestFileTask)taskToStore);
                break;
            case READY:
            case TERMINATED: //if the task was forcefully terminated, add it back into the queue of tasks waiting to run
            		...
            		TestFileTask fileTask = (TestFileTask)taskToStore;
    ...
    public String getResults() {
        ...
        if(!finishedTasks.isEmpty()) {
            ...
            for(TestFileTask task : finishedTasks) {
        ...
        if(!newTasks.isEmpty()) {
            ...
            for(TestFileTask task : newTasks) {
                retVal += task.getResults() + "\n";
            }
        }
        ...
        if(!problematicTasks.isEmpty()) {
            ...
            for(TestFileTask task : problematicTasks) {
                retVal += task.getResults() + "\n";
            }
        }
